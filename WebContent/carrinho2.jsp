...
<html>
   <head>
	...
	<body>
		...
		<c:choose>
			<c:when test="${requestScope.finalizado==1}">
				<font color="red">
		    ...
		    <c:otherwise>
				<form method="post" action="/servlet-01/Carrinho">
					<c:forEach items="${requestScope.produtos}" var="produto" varStatus="loop">
						...
						<fmt:formatNumber value="${produto.preco}" type="currency" currencySymbol="R$"/> x ${requestScope.qtd[loop.index]} = 
						<fmt:formatNumber value="${produto.preco * requestScope.qtd[loop.index]}" type="currency" currencySymbol="R$"/>
						Quantidade:<input name="qtd${produto.produto}" type="number" value="${requestScope.qtd[loop.index]}" />
						<br/><br/>
					</c:forEach>
